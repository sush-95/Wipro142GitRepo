...
namespace Read_File_Processor
{
    public class JsonCreater
    {
        ...
        public string getDetails_FreshCase(string resumeNumber, string candidateName, string MessageId,string sublogin, string ServiceId = "FreshCase")
        {
            string header = getHeader("CSPi Touchless App", Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), "query", "FreshCase", "Download Data from Customer Portal", Guid.NewGuid().ToString(), Guid.NewGuid().ToString(), "1.0.0", "1", "no", "jwt.token");
            string status = getStatus("value", "Desc", "details");
            ...
            string data = "\"data\":[{";
            data = data + "\"taskName\":\"downloadCaseData\",";
            data = data + "\"taskId\":\"" + Guid.NewGuid() + "\",";
            data = data + "\"taskSerialNo\":1,";
            data = data + "\"taskSpecs\":{\"downloadData\":{\"customerName\":\"Wipro Technologies\",\"resumeNumber\":\"" + resumeNumber.Replace("\"", "").Trim() + "\",\"subLogin\":\""+ sublogin + "\",\"candidateName\":\"" + candidateName.Replace("\"", "").Trim() + "\"}}";
            data = data + "}]";
        ...
        {
            string data = "\"data\":[{";
            data = data + "\"taskName\":\"downloadData\",";
            data = data + "\"taskId\":\"" + Guid.NewGuid() + "\",";
            data = data + "\"taskSerialNo\":1,";
            data = data + "\"taskSpecs\":{\"downloadData\":{\"customerName\":\"Wipro Technologies\",\"action\":\"download\",\"dataType\":\"excel\"}}";
            data = data + "}]";
            return data;
        }
