    ...
    public class ExecuteProcess
    {
        ...
        public string Execute_Excel_YetToStart_Process_Download()
        {
            ...
            try
            {
                        ...
                        if (details == "Campus type excel is downloaded")
                        {
                            ...
                            if (!string.IsNullOrEmpty(path))
                            {
                                ...
                                foreach (DataRow item in dt.Rows)
                                {
                                    ...
                                    {
                                        throw new Exception(ex.Message + "  Sublogin:Campus");
                                    }
                                }
                            ...
                            if (!string.IsNullOrEmpty(path))
                            {
                                ...
                                foreach (DataRow item in dt.Rows)
                                {
                                    try
                                    {
                                        ...
                                        string Insuff_Raised_Remarks = string.Empty;
                                        try
                                        {
                                            Insuff_Raised_Remarks = item["Insuff Raised Remarks"].ToString().Replace("'", "");
                                        }
                                        catch (Exception ex)
                                        {
                                            try
                                            {
                                                Insuff_Raised_Remarks = item["Insuff Cleared Remarks"].ToString().Replace("'", "");
                                            }
                                            catch { }
                                        }
                                    ...
                                    {
                                        throw new Exception(ex.Message + "  Sublogin:GSH");
                                    }
                        ...
                        else if (details == "Internal type excel is downloaded")
                        {
                            ...
                            if (!string.IsNullOrEmpty(path))
                            {
                                ...
                                foreach (DataRow item in dt.Rows)
                                {
                                    ...
                                    string FatherName = item["Father Name"].ToString().Replace("'", "");
                                    string DateofBirth = item["Date Of Birth"].ToString().Replace("'", "");
                                    string ContratNumber = item["Contact Number"].ToString().Replace("'", "");
                                    string EmailId = item["Email Id"].ToString().Replace("'", "");
                                    string Country = item["Country"].ToString().Replace("'", "");
                                    string Geo = item["Geo"].ToString().Replace("'", "");
                                    string Qualification = item["Qualification"].ToString().Replace("'", "");
                                    ...
                                    string PriorityFlag = item["Priority Flag"].ToString().Replace("'", "");
                                    string ActiveFlag = item["Active Flag"].ToString().Replace("'", "");
                                    string BGV_STATUS = item["BGV_STATUS"].ToString().Replace("'", "");
                                    string BGV_FINAL_STATUS = item["BGV_Final_status"].ToString().Replace("'", "");



                                    int res = objDML.InsertWiproInternalDetails(EmployeeNumber, EmployeeName, DateOfJoining, FatherName, DateofBirth, ContratNumber,
                                           EmailId, Country, Geo, Qualification, UniversityName, CollegeName, ModeOfEducation, YearOfPassing, AccountName,
                                           AgencyNumber, BGVInitiatedDate, BGVInsuffRaisedDate, BGVInsuffClearedDate, BGVAssignedDate, BGVQCRejectedDate,
                                           BGVSPOC, AG_REPORTED_STATUS_BGV, AG_REPORTED_DATE_BGV, SLA, AgencytoreverifyDate, AgencytoreverifyRemarks,
                                           AgencyPlannedClosureDate, PriorityFlag, ActiveFlag, BGV_STATUS, BGV_FINAL_STATUS);

                                    if (res == 2)
                                    {
                                        SendCaseDataToRabitMQ("Internal", EmployeeNumber, EmployeeName);
                                    }
                                }
                            }
                            objDML.Update_Response_Status(ResId);
                        ...
                        else if (details == "Lateral type excel is downloaded")
                        {
                            ...
                            if (!string.IsNullOrEmpty(path))
                            {
                                ...
                                foreach (DataRow item in dt.Rows)
                                {
                                    ...
                                    string priorityflag = item["priority flag"].ToString().Replace("'", "");
                                    string Divison = item["Divison"].ToString().Replace("'", "");
                                    string BGV1_status = item["BGV1_status"].ToString().Replace("'", "");
                                    string BGV2_status = item["BGV2_status"].ToString().Replace("'", "");
                                    string BGV1_Final_status = item["BGV1_Final_status"].ToString().Replace("'", "");
                                    string BGV2_Final_status = item["BGV2_Final_status"].ToString().Replace("'", "");
                ...
                List<tbl_wipro_campus_data> list = obj.Get_UnProcessedRequests();
                    ...
                    if (item.ResumeID != "No records Found")
                        if (!obj.IsCampusDuplicate(item.ResumeID))
                        {
                            ...
                            if (iDML == 1)
                            {
                                ...
                                bool ret = Write_JSON_TO_RABBIT_MQ(output);
                                objDML.Add_Exception_Log("Wipro: 1st attempt for Resume No : " + item.ResumeID + " Sub- Login : Campus has been sent to rabbitMQ ", item.ResumeID);
                                output = ret ? "Success" : "Failed";
                            }
                        }
                ...
                List<tbl_wipro_gsh_data> list1 = obj.Get_UnProcessedGSHRequests();
                    ...
                    if (item.ResumeNumber != "No records Found")
                        if (!obj.IsGSHDuplicate(item.ResumeNumber))
                        {
                            ...
                            if (iDML == 1)
                            {
                                ...
                                bool ret = Write_JSON_TO_RABBIT_MQ(output);
                                objDML.Add_Exception_Log("Wipro: 1st attempt for Resume No : " + item.ResumeNumber + " Sub- Login : GSH has been sent to rabbitMQ ", item.ResumeNumber);
                                output = ret ? "Success" : "Failed";
                            }
                        }
                    objDML.UpdateWiproGSHDetails(item.ResumeNumber, item.CandidateName);
                }
                List<tbl_wipro_internal_data> list2 = obj.Get_UnProcessedInternalRequests();
                    ...
                    if (item.EmployeeNumber != "No records Found")
                        if (!obj.IsInternalDuplicate(item.EmployeeNumber))
                        {
                            ...
                            if (iDML == 1)
                            {
                                ...
                                bool ret = Write_JSON_TO_RABBIT_MQ(output);
                                objDML.Add_Exception_Log("Wipro: 1st attempt for Resume No : " + item.EmployeeNumber + " Sub- Login : Internal has been sent to rabbitMQ ", item.EmployeeNumber);
                                output = ret ? "Success" : "Failed";
                            }
                        }
                ...
                List<tbl_wipro_lateral_data> list3 = obj.Get_UnProcessedLateralRequests();
                    ...
                    if (item.ResumeNumber != "No records Found")
                        if (!obj.IsLateralDuplicate(item.ResumeNumber))
                        {
                            ...
                            if (iDML == 1)
                            {
                                ...
                                bool ret = Write_JSON_TO_RABBIT_MQ(output);
                                objDML.Add_Exception_Log("Wipro: 1st attempt for Resume No : " + item.ResumeNumber + " Sub- Login : Lateral has been sent to rabbitMQ ", item.ResumeNumber);
                                output = ret ? "Success" : "Failed";
                            }
                        }
                    objDML.UpdateWiproLateralDetails(item.ResumeNumber, item.CandidateName);
                }
                List<tbl_wipro_contract_data> list4 = obj.Get_UnProcessedContractRequests();
                    ...
                    if (item.ResumeNumber != "No records Found")
                        if (!obj.IscontractDuplicate(item.ResumeNumber))
                        {
                            ...
                            if (iDML == 1)
                            {
                                ...
                                bool ret = Write_JSON_TO_RABBIT_MQ(output);
                                objDML.Add_Exception_Log("Wipro: 1st attempt for Resume No : "+item.ResumeNumber+ " Sub- Login : Contract has been sent to rabbitMQ ",item.ResumeNumber);
                                output = ret ? "Success" : "Failed";
                            }
                        }
            ...
            catch (Exception ex)
            {
                ...
                int iException = objDML.Add_Exception_Log("Wipro exception : " + ex.Message, "Execute_Excel_YetToStart_Process_Download");
        ...
        public string Write_JSON_TO_Download()
        {
            ...
            try
            {
                ...
                if (lstObj.Count > 0)
                {
                    ...
                    List<tbl_config_value> lstConfig = objGet.Get_Cofig_Details("WiproDOWNLOADREQUEST");
                    foreach (var ob in lstConfig)
                    {
